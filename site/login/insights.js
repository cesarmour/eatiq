...
function show(r,goal,recs={suggestions:[],restaurants:[],reason:'Sem histórico neste filtro.'}){
 ...
 const dish=d=>`<article class="recommendation-card"><div class="recommendation-cover" aria-hidden="true"><span>${esc(d.format==='lamen'?'Lámen':d.format==='grelhados'?'À mesa':d.store.category||'À sua escolha')}</span></div><div class="recommendation-body"><span class="function-label">${esc(d.label)}</span><p class="meta">${esc(d.store.name)} · iFood</p><h3>${esc(d.name)}</h3><p class="recommendation-reason">${esc(d.reason)}. ${esc(d.purpose)}</p>${d.description?`<p class="meta recommendation-description">${esc(d.description.slice(0,180))}</p>`:''}<div class="recommendation-footer">${d.nutrition?`<p class="meta">Estimativa por item: ${num(d.nutrition.kcal)} kcal · ${num(d.nutrition.p)} g proteína · ${num(d.nutrition.c)} g carboidratos · ${num(d.nutrition.f)} g gordura</p>`:''}<p>${esc(rating(d))}</p><p class="meta">${esc(valid(d.price)?money(+d.price)+' na coleta':'Preço não informado')}${d.choices?' · escolhas no cardápio':''}</p>${link(d)}</div></div></article>`;
...
function recommend(orders,items,menu,goal){return window.EatIQTaste.recommend(orders,items,menu,goal,{budget:document.getElementById('recommendationBudget')?.value||50,estimate:d=>{const key=d.storeId+'|'+d.id;if(!nutritionCache.has(key))nutritionCache.set(key,window.EatIQEngine.estimate(d.name,d.description,1,'restaurante',{}));return nutritionCache.get(key)}})}
async function render(args){
 ...
 if(!orders.length){show(aggregate([],[],goal),goal);return}if(key===cacheKey&&cache){show(aggregate(orders,cache,goal),goal,recommend(orders,cache,orders.some(o=>norm(o.app)==='ifood')?catalog:null,goal));return}
