...
function show(r,goal,recs={suggestions:[],restaurants:[],reason:'Sem histórico neste filtro.'}){
 ...
 const dish=d=>`<article class="recommendation-card"><div class="recommendation-cover" aria-hidden="true"><span>${esc(d.format==='lamen'?'Lámen':d.format==='grelhados'?'À mesa':d.store.category||'À sua escolha')}</span></div><div class="recommendation-body"><span class="function-label">${esc(d.label)}</span><p class="meta">${esc(d.store.name)} · iFood</p><h3>${esc(d.name)}</h3><p class="recommendation-reason">${esc(d.label==='Seu favorito'?d.times+' pedidos no seu histórico':d.reason)}</p>${d.clinical?.active?`<p class="recommendation-reason clinical-reason"><b>${d.clinical.adjustment>0?'Priorizado pelos seus marcadores':d.clinical.adjustment<0?'Ponto de atenção nos seus marcadores':'Exames considerados'}</b><br>${d.clinical.reasons.map(esc).join('<br>')}</p>`:''}<div class="recommendation-footer">${d.nutrition?`<p class="meta">Estimativa por item: ${num(d.nutrition.kcal)} kcal · ${num(d.nutrition.p)} g proteína · ${num(d.nutrition.c)} g carboidratos · ${num(d.nutrition.f)} g gordura</p>`:''}<p>${esc(rating(d))}</p><p class="meta">${esc(valid(d.price)?money(+d.price)+' na coleta':'Preço não informado')}${d.choices?' · escolhas no cardápio':''}</p>${link(d)}</div></div></article>`;
 ...
 document.getElementById('insightsStatus').textContent=r.orderCount+' pedidos de restaurantes · '+r.itemCount+' itens distintos no filtro. '+(recs.collectedAt?'Cardápios e avaliações: '+recs.collectedAt+'. ':'')+(window.EatIQClinical?.summary(lastArgs?.labs)||'')+' Afinidade baseada no quiz e no histórico disponíveis. Preços, entrega e disponibilidade atuais devem ser conferidos no iFood.';
}
function recommend(orders,items,menu,goal){return window.EatIQTaste.recommend(orders,items,menu,goal,{quiz:lastArgs?.quiz,labs:lastArgs?.labs,budget:document.getElementById('recommendationBudget')?.value||50,estimate:d=>{const key=d.storeId+'|'+d.id;if(!nutritionCache.has(key))nutritionCache.set(key,window.EatIQEngine.estimate(d.name,d.description,1,'restaurante',{}));return nutritionCache.get(key)}})}
