...
async function importHealth(){const f=$('hkFile').files[0];if(!f||healthBusy)return;healthBusy=true;const st=$('hkStatus');st.textContent='Lendo '+f.name+'...';
 try{const result=await EatIQHealth.read(f);const rows=result.rows.map(r=>({...r,user_id:USER.id}));
  ...
  rows.forEach(r=>HD[r.dia]=r);st.textContent=`Pronto: ${rows.length} dias importados. Veja a cobertura e os cruzamentos em Pedidos × Health.`;setupHealth();render();
...
function render(){if(window.__rawO)ALL=prep(window.__rawO);const O=filtered();const meals=O.filter(o=>o.meal);const pf=profile();if(USER&&window.EatIQInsights){const selected=new Set(O.map(o=>String(o.id)));window.EatIQInsights.render({orders:(window.__rawO||[]).filter(o=>selected.has(String(o.id))),userId:USER.id,api,quiz:PROF.quiz_gostos,labs:labVals()})}
  ...
  // Each Health card has its own data requirements; active energy is not a shared gate.
  if(window.EatIQHealthInsights){const ids=new Set(O.map(o=>String(o.id)));window.EatIQHealthInsights.render((window.__rawO||[]).filter(o=>ids.has(String(o.id))),HD,state.t)}
  // --- exames
  renderLabs(meals);
  // --- tabela
